...
var custom = (function() {
  ...
  */
  ...
  function priceFormat (product) {
    ...
    product.salePrice = replaceChineseYenSign(product.salePrice);
    product.salePrice = removeCents(product.salePrice); 
    if(!product.regularPrice) return;
    product.regularPrice = replaceChineseYenSign(product.regularPrice);
    product.regularPrice = removeCents(product.regularPrice); 
  }
  ...
  function removeCents (salePrice) {
    if(! salePrice) return;
    var matcher = /(\.|\,)0{2}(?![\d])/;
    return salePrice.replace(matcher, '');
  };
  function removeCents (regularPrice) {
    if(! regularPrice) return;
    var matcher = /(\.|\,)0{2}(?![\d])/;
    return regularPrice.replace(matcher, '');
  };
